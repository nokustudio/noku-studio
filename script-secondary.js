(function () {
  ...
  camera.position.set(0, 0.6, 4.5); // CONFIG: camera_y
  ...
  let introModelX = 0.0;
  let introModelY = 0.6150; // CONFIG: fallback_y
  let introModelScale = 0.4200; // CONFIG: fallback_scale
  let introModelRotY = -0.872665; // CONFIG: fallback_rot_y
  ...
  function evaluateScrollCalculations() {
    ...
    if (scrollY <= PIN_SCROLL) {
      ...
      targetRotY = -0.872665; // CONFIG: rot_y_phase1
      ...
      if (window.innerWidth > 1024) {
        ...
        const rotYKeyframes = [
          [0.0, -0.872665], // CONFIG: rot_y_phase2_desktop
          [0.35, 0.65 * Math.PI],
          [0.65, 1.35 * Math.PI],
          [1.0, 2.0 * Math.PI]
        ];
        ...
        const rotYKeyframesMobile = [
          [0.0, -0.872665], // CONFIG: rot_y_phase2_mobile
          [0.35, 0.65 * Math.PI],
          [0.65, 1.35 * Math.PI],
          [1.0, 2.0 * Math.PI]
        ];
      ...
      if (scrollProgress < blendRange) {
        ...
        targetRotY = (-0.872665) * (1 - blendEased) + baseRotY * blendEased; // CONFIG: rot_y_blend
      } else {
        targetPosX = basePosX;
        targetPosY = basePosY;
